  ...
  translation: {
    ...
    calculator: {
      ...
      warnings: {
        ...
        equityShortfall: 'הון עצמי נמוך מהנדרש — נדרש לפחות {{required}} ({{requiredPercent}}%)',
        ...
        closingCostsTotal: '• סה"כ עלויות נלוות ומיסים: <0>{{total}}</0>',
        equityTotalPrefix: '• סה"כ נדרש בהתחלה (הון עצמי + עלויות נלוות): ',
        ltv: 'שיעור המימון ({{percent}}%) חורג מהמותר בבנק ישראל ל{{purpose}} (עד {{limit}}%) \n על נכס בשווי {{value}} ניתן לקחת משכנתא של עד {{maxLoan}}',
        dti: 'ההחזר החודשי ({{payment}}) מחייב הכנסה פנויה חודשית של {{minIncome}} לפחות\nלפי תקרת 50% הנהוגה בבנקים — ההכנסה הפנויה חייבת להיות לפחות פי 2 מההחזר החודשי\n(ההכנסה שלכם נמוכה ב-{{shortfall}}% מהנדרש לתשלומים)',
