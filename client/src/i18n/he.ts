  ...
  translation: {
    ...
    calculator: {
      ...
      warnings: {
        ...
        ltvMaxLoan: 'על נכס בשווי <0>{{value}}</0> ניתן לקבל משכנתא של עד <1>{{maxLoan}}</1>',
        ltvOk: 'שיעור המימון (<0>{{percent}}</0>%) עומד במותר ל{{purpose}} (עד <1>{{limit}}</1>%)',
        dti: 'ההחזר החודשי (<0>{{payment}}</0>) נמוך ב-<1>{{shortfall}}</1>% מהנדרש. הבנק יבקש הכנסה חודשית פנויה של לפחות <2>{{minIncome}}</2>',
        dtiOk: 'ההחזר החודשי (<0>{{payment}}</0>) עומד בתקרת 50% מההכנסה הפנויה',
