  ...
  translation: {
    ...
    calculator: {
      ...
      warnings: {
        ...
        equityShortfall: 'Equity is below the required share — at least {{required}} ({{requiredPercent}}%) is needed',
        closingCosts: '• Side costs (attorney, registration & surveyor): <0>{{amount}}</0> (<1>{{percent}}</1>%)',
        purchaseTaxNone: '• Purchase tax: not applicable for a {{purpose}} up to <0>{{threshold}}</0>',
        ...
        closingCostsTotal: '• Total side costs & taxes: <0>{{total}}</0>',
        equityTotalPrefix: '• Total needed upfront (equity + side costs): ',
        ltv: 'The financing ratio ({{percent}}%) exceeds the Bank of Israel limit for a {{purpose}} (up to {{limit}}%) \n On a property worth {{value}} the maximum mortgage is {{maxLoan}}',
        dti: 'The monthly payment ({{payment}}) requires a net free monthly income of at least {{minIncome}}\nUnder the 50% ceiling used by banks — free income must be at least double the monthly payment\n(Your income is {{shortfall}}% lower than required for these payments)',
