  ...
  translation: {
    ...
    calculator: {
      ...
      warnings: {
        ...
        ltvMaxLoan: 'On a property worth <0>{{value}}</0> the maximum mortgage is <1>{{maxLoan}}</1>',
        ltvOk: 'The financing ratio (<0>{{percent}}</0>%) is within the limit for a {{purpose}} (up to <1>{{limit}}</1>%)',
        dti: 'The monthly payment (<0>{{payment}}</0>) is <1>{{shortfall}}</1>% below required. The bank will ask for a net free monthly income of at least <2>{{minIncome}}</2>',
        dtiOk: 'The monthly payment (<0>{{payment}}</0>) is within the 50% ceiling of free income',
